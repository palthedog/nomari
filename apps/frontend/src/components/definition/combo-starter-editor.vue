  ...
  <div class="combo-starter-editor">
    ...
    <div class="section">
      ...
      <div class="form-group">
        <v-text-field v-model="model.name" label="名前" density="compact" placeholder="例: 中パン始動" variant="outlined"
          hide-details />
      </div>
      <div class="form-group">
        <v-text-field v-model="model.description" label="説明" density="compact" placeholder="例: 中パンヒット確認からのコンボ"
    ...
    <div class="section">
      <h4>コンボルート</h4>
      <div v-for="(route, routeIndex) in model.routes" :key="routeIndex" class="route-section">
        <div class="route-header">
          <v-text-field v-model="route.name" label="ルート名" density="compact" placeholder="例: ノーゲージ, OD確認, SA〆"
            variant="outlined" hide-details class="route-name-input" />
          <button type="button" class="delete-route-btn" @click="removeRoute(routeIndex)">
            削除
          </button>
        </div>
        ...
        <div class="route-subsection">
          <div class="subsection-header">必要ゲージ</div>
          <div v-for="(req, reqIndex) in route.requirements" :key="reqIndex" class="resource-row">
            <v-select :model-value="req.resourceType" :items="gaugeTypeItems" item-title="title" item-value="value"
              density="compact" variant="outlined" hide-details class="resource-select"
              @update:model-value="(value: number) => updateRequirement(routeIndex, reqIndex, 'type', value)" />
            <input type="number" :value="req.value" placeholder="必要量"
              @input="updateRequirement(routeIndex, reqIndex, 'value', parseFloat(($event.target as HTMLInputElement).value) || 0)">
            <button type="button" class="delete-row-btn" @click="removeRequirement(routeIndex, reqIndex)">
              削除
            </button>
          </div>
          <button type="button" class="add-row-btn" @click="addRequirement(routeIndex)">
            必要ゲージを追加
          </button>
        </div>
        ...
        <!-- Consumptions -->
        <div class="route-subsection">
          <div class="subsection-header">消費リソース</div>
          <div v-for="(cons, consIndex) in route.consumptions" :key="consIndex" class="resource-row">
            <v-select :model-value="cons.resourceType" :items="consumptionTypeItems" item-title="title"
              item-value="value" density="compact" variant="outlined" hide-details class="resource-select"
              @update:model-value="(value: number) => updateConsumption(routeIndex, consIndex, 'type', value)" />
            <input type="number" :value="cons.value" placeholder="消費量"
              @input="updateConsumption(routeIndex, consIndex, 'value', parseFloat(($event.target as HTMLInputElement).value) || 0)">
            <button type="button" class="delete-row-btn" @click="removeConsumption(routeIndex, consIndex)">
              削除
            </button>
          </div>
          <button type="button" class="add-row-btn" @click="addConsumption(routeIndex)">
            消費リソースを追加
          </button>
        </div>
        ...
        <!-- Next Situation -->
        <div class="route-subsection">
          <div class="subsection-header">遷移先</div>
          <v-select :model-value="route.nextSituationId" :items="nextSituationItems" item-title="title"
