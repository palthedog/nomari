...
export class CFRSolver {
    ...
    private buildInternalTree(): void {
        ...
        this.collectAllNodes(this.gameTree.root);

        // Then build CFR nodes
        this.buildCFRNode(this.gameTree.root);
    }
    ...
    private findNodeById(nodeId: string): Node | undefined {
        ...
        return this.searchNode(this.gameTree.root, nodeId);
    }
    ...
    public solve(iterations: number = 1000): void {
        const root = this.nodes.get(this.gameTree.root.id);
        if (!root) {
    ...
    public getRootStrategy(): Map<string, number> | null {
        return this.getAverageStrategy(this.gameTree.root.id);
    }
}
